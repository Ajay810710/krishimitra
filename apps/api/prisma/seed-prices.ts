/**
 * Seeds 90 days of synthetic mandi price data for all crop×mandi combinations.
 * Prices follow realistic seasonal patterns with noise.
 */
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// Base modal prices (₹/quintal) per crop — realistic Indian market ranges
...
function generatePrice(base: number, dayOffset: number): { min: number; max: number; modal: number } {
...
async function main() {
  ...
  for (const crop of crops) {
    const baseName = Object.keys(CROP_BASE_PRICES).find(
      (k) => crop.name.toLowerCase().includes(k.toLowerCase()) || k.toLowerCase().includes(crop.name.toLowerCase())
    );
